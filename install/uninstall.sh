...
step11=`rm -rf /usr/local/cpanel/base/frontend/paper_lantern/microweber`
if [ -n "$step11" ]; then
    echo "Unable to complete step 1-1"
fi
step11_1=`rm -rf /usr/local/cpanel/base/frontend/jupiter/microweber`
if [ -n "$step11_1" ]; then
    echo "Unable to complete step 1-1_1"
fi
